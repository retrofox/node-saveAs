...
exports.save = function(options, folder, fn){
    ...
    var handler = new htmlParser.DefaultHandler(function(err, dom) {
      if (err) {
        ...
        for (i = 0; i < images.length; i++) {
          ...
          if(images[i].attribs 
          //   && images[i].attribs.href.search(/^http(s:|:)\/\/|^mailto:|^\/$|^#/) < 0
          ) {
  ...
  getRemoteData(this.options.request, function(err, data){
    ...
    createFoler(folder, function(){
      ...
      fs.writeFile(filename, data, 'utf8', function (err) {
        ...
        gutPage(data, {}, function(err, resources){
          ...
          resources.images.forEach(function(image, iI) {
            ...
            getRemoteData(self.options.request, 'binary', function(err, data, req, res){
