              ...
              resources.stylesheets.forEach(function(css, iI) {
                ...
                getRemoteData(self.options.request, 'utf8', function(err, cssData, req, res){
                  var spl = css.path.split('/')
                    , cssUri = spl[spl.length-1]
                    , cssName = cssUri.split('?')[0]
                  ...
                  if(imagesInCss) {
                    // counter plus all css imgages.
//                    counter = counter + imagesInCss.length;
                    delete self.options.request.agent;

                    // save all css images into css folder
                    var cssImagesFolder = stylesheetsFolder + '/images';
                    createFolder(cssImagesFolder, function() {

                      imagesInCss.forEach(function(imageInCss, iI) {
                        var hasC = imageInCss.split(/['|"]/).length > 1
                          , imgUrl = imageInCss.substr(hasC ? 5 : 4, imageInCss.length - (hasC ? 7: 5))
                          , urlLink = gutLink(imgUrl)

                        self.options.request.path = css.path.substr(0, css.path.length - cssUri.length) + imgUrl;


/*                      console.log("-- self.options.request ----------------------------------------------------");
                        console.log(self.options.request);
                        console.log("---------------------------------------------------- self.options.request --");
  */                
                        var cssImgSplit = imgUrl.split('/')
                          , cssImageName = cssImgSplit[cssImgSplit.length-1]
                          , cssImagePath= cssImagesFolder + '/' + cssImageName;

                      console.log("-- cssImagePath ----------------------------------------------------");
                      ...
                      console.log("---------------------------------------------------- cssImagePath --");
                        ...
                        getRemoteData(self.options.request, 'binary', function(err, imgBuffer, req, res){
                          saveFile(cssImagePath, imgBuffer, 'binary', function(){
                            console.log('saved -> ', cssImagePath);
